...
_a = ConfigurationLoader;
ConfigurationLoader.load = (cmdArgs) => {
    return _a.loadConfigFile(cmdArgs);
};
ConfigurationLoader.loadConfigFile = (cmdArgs) => {
    let data;
    try {
...
ConfigurationLoader.loadConfigFromCustomPath = (path) => {
    let data;
    if (fs.existsSync(path)) {
        ...
        throw new Error(`Configuration path is not valid. The file does not exists. Path: ${path}`);
    }
    return data;
};
